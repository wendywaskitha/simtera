...
class PerformanceMetricsWidget extends Widget
{
    ...
    public function getViewData(): array
    {
        ...
        $teraSah = HasilTera::whereBetween('tanggal_tera', [$startMonth, $endMonth])
                             ->where('hasil', 'Sah')->count();

        $tingkatKeberhasilan = $totalTera > 0 ? round(($teraSah / $totalTera) * 100, 1) : 0;
        ...
        $petugasPerformance = Petugas::aktif()
                                    ...
                                    ->withCount(['hasilTeras as tera_sah' => function ($query) use ($startMonth, $endMonth) {
                                        $query->whereBetween('tanggal_tera', [$startMonth, $endMonth])
                                              ->where('hasil', 'Sah');
                                    }])
                                    ->having('total_tera', '>', 0)
                                    ...
                                    ->map(function ($petugas) {
                                        $petugas->success_rate = $petugas->total_tera > 0 ?
                                            round(($petugas->tera_sah / $petugas->total_tera) * 100, 1) : 0;
                                        return $petugas;
                                    });

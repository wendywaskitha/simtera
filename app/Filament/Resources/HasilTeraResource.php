...
class HasilTeraResource extends Resource
{
                                ...
                                ->schema([
                                    Forms\Components\Select::make('permohonan_tera_id')
                                        ...
                                        ->afterStateUpdated(function (Forms\Set $set, $state) {
                                            if ($state) {
                                                $permohonan = PermohonanTera::with(['uttp.pemilik'])->find($state);
                                                if ($permohonan) {
                                        ...
                                        ->getOptionLabelFromRecordUsing(fn (PermohonanTera $record) =>
                                            "{$record->nomor_permohonan} - {$record->uttp?->pemilik?->nama} ({$record->status})")
                                        ->placeholder('Cari berdasarkan nomor permohonan')
                                        ->helperText('Pilih permohonan tera yang sudah dijadwalkan')
                                        ->options(function () {
                                            return PermohonanTera::with(['uttp.pemilik'])
                                                ->whereIn('status', ['Dijadwalkan'])
                                                ->get()
                                                ->mapWithKeys(function ($permohonan) {
                                                    $namaPemilik = $permohonan->uttp?->pemilik?->nama ?? 'Tanpa Pemilik';
                                                    return [$permohonan->id => "{$permohonan->nomor_permohonan} - {$namaPemilik}"];
                                                });
                                        }),
                                    ...
                                    Forms\Components\Placeholder::make('permohonan_info')
                                        ...
                                        ->content(function (Forms\Get $get) {
                                            ...
                                            $permohonan = PermohonanTera::with(['uttp.jenisUttp', 'uttp.desa', 'uttp.pemilik'])->find($get('permohonan_tera_id'));
                                    ...
                                    Forms\Components\Grid::make(2)
                                        ->schema([
                                            Forms\Components\TextInput::make('nomor_sertifikat')
                                                ...
                                                ->visible(fn (Forms\Get $get) => $get('hasil') === 'Sah')
                                ...
                                ->schema([
                                    Forms\Components\Placeholder::make('review_summary')
                                        ...
                                        ->content(function (Forms\Get $get) {
                                            ...
                                            $permohonan = PermohonanTera::with(['uttp.pemilik'])->find($get('permohonan_tera_id'));
                                            $hasil = $get('hasil');
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->with(['uttp.pemilik', 'permohonanTera', 'uttp.jenisUttp']))
            ->columns([
                ...
                Tables\Columns\BadgeColumn::make('hasil')
                    ...
                    ->colors([
                        'success' => 'Sah',
                        'danger' => ['Batal', 'Rusak', 'Tidak Layak'],
                    ])
                    ->icons([
                        'heroicon-o-check-circle' => 'Sah',
                        'heroicon-o-x-circle' => ['Batal', 'Rusak', 'Tidak Layak'],
                    ]),
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                    ...
                    Tables\Actions\Action::make('regenerate_certificate')
                        ...
                        ->visible(fn (HasilTera $record) => $record->hasil === 'Sah'),
                ...
                Tables\Actions\BulkActionGroup::make([
                    ...
                    Tables\Actions\BulkAction::make('bulk_download_certificates')
                        ...
                        ->action(function ($records) {
                            $validRecords = collect($records)->filter(fn ($record) => $record->hasil === 'Sah' && $record->nomor_sertifikat);
                        ...
                        ->modalDescription('Download sertifikat untuk semua hasil tera yang Sah?'),
                    Tables\Actions\BulkAction::make('bulk_certificate')
                        ...
                        ->action(function ($records) {
                            ...
                            $collection->each(function ($record) use (&$generated, &$failed) {
                                if ($record && $record->hasil === 'Sah' && $record->nomor_sertifikat) {
                                    try {
                                        $generated++;
                                    } catch (\Exception $e) {
                                        $failed++;
                            ...
                            if ($generated > 0) {
                                ...
                                Notification::make()
                                    ...
                                    ->body('Pastikan hasil tera berstatus "Sah" dan memiliki nomor sertifikat')
                                    ->warning()
                                    ->send();
                            }
                        })
                        ...
                        ->modalDescription('Sistem akan membuat file PDF sertifikat untuk semua hasil tera yang Sah. Proses ini mungkin memakan waktu beberapa saat.'),
                ]),
            ])
    ...
    {
        return static::getModel()::where('hasil', 'Sah')->count();
    }

    public static function getGlobalSearchResultTitle(Model $record): string
    {
        return ($record->nomor_sertifikat ?? 'Hasil Tera') . ' - ' . ($record->uttp?->pemilik?->nama ?? 'Tanpa Pemilik');
    }

    public static function getGlobalSearchResultDetails(Model $record): array
    {
        return [
            'UTTP' => $record->uttp?->kode_uttp ?? 'Tidak ada',
            'Hasil' => $record->hasil,
            'Tanggal' => $record->tanggal_tera?->format('d M Y') ?? 'Tidak ada',
        ];
    }
}
